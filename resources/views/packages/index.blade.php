        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto align-items-center">
                    ...
                    <li class="nav-item"><a class="nav-link active fw-semibold mx-2 text-danger"
                            href="/packages">Packages</a></li>
                    <li class="nav-item"><a class="nav-link fw-semibold mx-2" href="#">Destinations</a></li>
                    @guest
                        <li class="nav-item ms-3"><a class="btn btn-outline-danger rounded-pill px-4"
                                href="/login">Login</a></li>
                    @else
                        <li class="nav-item ms-3"><a class="btn btn-danger rounded-pill px-4" href="/admin">Dashboard</a>
                        </li>
                    @endguest
                </ul>
            </div>
            ...
            <div class="col-lg-3">
                <form action="/packages" method="GET" id="filterForm">
                    ...
                    <div class="filter-sidebar">
                        ...
                        <div class="mb-4">
                            ...
                            @foreach(['1-3' => '1-3 Days', '4-6' => '4-6 Days', '7-9' => '7-9 Days', '10+' => '10+ Days'] as $val => $label)
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="duration" value="{{ $val }}"
                                        id="dur_{{ $val }}" {{ request('duration') == $val ? 'checked' : '' }}
                            ...
                            @foreach($categories as $cat)
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="category" value="{{ $cat }}"
                                        id="cat_{{ $cat }}" {{ request('category') == $cat ? 'checked' : '' }}
                                        onchange="this.form.submit()">
                        ...
                        <!-- Country -->
                        <div class="mb-4">
                            <h6 class="fw-bold mb-3 small text-uppercase text-muted">Country</h6>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="destination" class="form-select form-select-sm rounded-3"
                                onchange="this.form.submit()">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <select class="form-select form-select-sm rounded-3" style="width: auto;"
                            onchange="window.location.href='/packages?'+new URLSearchParams({...Object.fromEntries(new URLSearchParams(location.search)), sort: this.value})">
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mt-auto">
                                        <div class="duration-tag">
                                            <i class="bi bi-clock me-1"></i> {{ $pkg->duration_days }}D /
                                            {{ $pkg->duration_nights }}N
                                        </div>
                                        <div class="text-end">
                                            <div class="price-tag">
                                                {{ \App\Helpers\CurrencyHelper::format($pkg->price, $pkg->currency ?? 'MYR') }}
                                            </div>
                                            <div class="small text-muted" style="font-size: 0.7rem;">per person</div>
                                        </div>
                                    </div>
                                    <a href="/packages/{{ $pkg->slug }}"
                                        class="btn btn-outline-danger w-100 rounded-pill mt-4 fw-bold small">View
                                        Details</a>
                                </div>
                            </div>
                        </div>
