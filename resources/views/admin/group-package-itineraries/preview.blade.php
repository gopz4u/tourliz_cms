...
.daisy-view h1, .daisy-view h2, .daisy-view h3 {
...
<div class="daisy-view p-4 md:p-8 bg-base-200 min-h-screen rounded-2xl">
    <div class="flex flex-col md:flex-row justify-between items-center mb-10 gap-6 bg-base-100 p-8 rounded-3xl shadow-sm border border-base-200 relative overflow-hidden">
        ...
        <div class="relative z-10 w-full md:w-auto">
            ...
            <p class="text-base-content/60 mt-3 text-lg m-0 md:ml-16">Previewing itinerary for: <span class="font-bold text-neutral">{{ $package->name }}</span></p>
        </div>
        <div class="flex flex-col sm:flex-row flex-wrap gap-3 relative z-10 w-full md:w-auto items-center md:justify-end">
            <!-- Customer Actions -->
            <div class="join join-vertical sm:join-horizontal shadow-xl shadow-primary/20">
                <button onclick="shareItineraryWhatsapp('customer')" class="btn bg-[#25D366] hover:bg-[#1DBA55] join-item text-white border-0 px-6">
                    <i class="bi bi-whatsapp"></i> Customer WhatsApp
                </button>
                <a href="{{ route('admin.group-package-itineraries.pdf', ['id' => $package->id, 'for' => 'customer']) }}" class="btn btn-secondary join-item text-white border-0 px-6">
                    <i class="bi bi-file-pdf"></i> Customer PDF
                </a>
            </div>

            <!-- Company Actions -->
            <div class="join join-vertical sm:join-horizontal shadow-xl shadow-neutral/20">
                <button onclick="shareItineraryWhatsapp('company')" class="btn btn-neutral join-item text-white border-0 px-6">
                    <i class="bi bi-whatsapp"></i> Company WhatsApp
                </button>
                <a href="{{ route('admin.group-package-itineraries.pdf', ['id' => $package->id, 'for' => 'company']) }}" class="btn btn-primary join-item text-white border-0 px-6">
                    <i class="bi bi-file-pdf"></i> Company PDF
                </a>
            </div>

            <a href="{{ route('admin.group-package-itineraries.edit', $package->id) }}" class="btn btn-ghost hover:bg-base-200 transition-colors rounded-full px-6 border-base-300 w-full sm:w-auto">
                <i class="bi bi-pencil"></i> Edit
            </a>
        </div>
    </div>

    <!-- Full-width Itinerary Grid -->
    <div class="space-y-12">
                ...
                <div class="hero bg-base-100 rounded-3xl shadow-xl border border-base-200/50 py-24 object-cover relative overflow-hidden">
                    ...
                    <div class="hero-content text-center relative z-10">
                        <div class="max-w-md">
                            ...
                            <a href="{{ route('admin.group-package-itineraries.edit', $package->id) }}" class="btn btn-primary btn-lg rounded-full px-10 shadow-xl shadow-primary/30 text-white border-0 hover:-translate-y-1 transition-transform">Create Itinerary <i class="bi bi-arrow-right ml-2"></i></a>
                        </div>
                    </div>
                </div>
            @endforelse
        ...
        <!-- Footer Breakdown (Cost + Summary) -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 pt-12 pb-12 border-t border-base-200">
            <!-- Summary Stats Card -->
            <div class="card bg-gradient-to-br from-neutral to-neutral-focus text-neutral-content shadow-2xl rounded-3xl overflow-hidden border border-white/10 group h-full">
                <!-- Background decor -->
                ...
                <div class="card-body p-8 relative z-10">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="cost-row bg-black/20 hover:bg-black/30 p-4 rounded-xl transition-colors group/item">
                            <span class="flex items-center gap-3 text-xs font-semibold uppercase tracking-wider text-white/90 shrink-0"><div class="w-2.5 h-2.5 rounded-full bg-teal-400 shadow-[0_0_8px_rgba(45,212,191,0.5)]"></div> Accommodation</span>
                            <span class="font-mono px-2 py-1 rounded bg-black/40 font-bold text-white shadow-inner whitespace-nowrap text-base">{{ $costBreakdown['currency'] }} {{ number_format($costBreakdown['hotels'], 2) }}</span>
                        </div>
                        <div class="cost-row bg-black/20 hover:bg-black/30 p-4 rounded-xl transition-colors group/item">
                            <span class="flex items-center gap-3 text-xs font-semibold uppercase tracking-wider text-white/90 shrink-0"><div class="w-2.5 h-2.5 rounded-full bg-secondary shadow-[0_0_8px_rgba(16,185,129,0.5)]"></div> Transport</span>
                            <span class="font-mono px-2 py-1 rounded bg-black/40 font-bold text-white shadow-inner whitespace-nowrap text-base">{{ $costBreakdown['currency'] }} {{ number_format($costBreakdown['transport'], 2) }}</span>
                        </div>
                        <div class="cost-row bg-black/20 hover:bg-black/30 p-4 rounded-xl transition-colors group/item">
                            <span class="flex items-center gap-3 text-xs font-semibold uppercase tracking-wider text-white/90 shrink-0"><div class="w-2.5 h-2.5 rounded-full bg-accent shadow-[0_0_8px_rgba(139,92,246,0.5)]"></div> Activities</span>
                            <span class="font-mono px-2 py-1 rounded bg-black/40 font-bold text-white shadow-inner whitespace-nowrap text-base">{{ $costBreakdown['currency'] }} {{ number_format($costBreakdown['activities'], 2) }}</span>
                        </div>
                        <div class="cost-row bg-black/20 hover:bg-black/30 p-4 rounded-xl transition-colors group/item">
                            <span class="flex items-center gap-3 text-xs font-semibold uppercase tracking-wider text-white/90 shrink-0"><div class="w-2.5 h-2.5 rounded-full bg-orange-400 shadow-[0_0_8px_rgba(251,146,60,0.5)]"></div> Tickets</span>
                            <span class="font-mono px-2 py-1 rounded bg-black/40 font-bold text-white shadow-inner whitespace-nowrap text-base">{{ $costBreakdown['currency'] }} {{ number_format($costBreakdown['entry_tickets'], 2) }}</span>
                        </div>
                    </div>

                    <div class="mt-8 bg-black/30 rounded-2xl p-6 border border-white/5 shadow-inner">
                        <div class="text-xs font-bold text-white/50 uppercase tracking-widest mb-1 text-center">Total Estimated Cost</div>
                        <div class="text-center">
                            <span class="text-primary font-bold text-2xl">{{ $costBreakdown['currency'] }}</span>
                            <span class="text-5xl font-black font-mono text-white tracking-tighter ml-2">{{ number_format($costBreakdown['total'], 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Package Info Card -->
            <div class="card bg-base-100 shadow-xl shadow-base-200/50 border border-base-200/60 rounded-3xl h-full">
                <div class="card-body p-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 h-full">
                        <div class="flex items-center gap-4 bg-base-200/50 p-6 rounded-2xl hover:bg-base-200 transition-colors">
                            <div class="w-14 h-14 rounded-xl bg-white shadow-sm flex items-center justify-center text-primary shrink-0">
                                <i class="bi bi-clock-history text-2xl"></i>
                            </div>
                            <div>
                                <div class="text-xs font-bold uppercase tracking-widest text-base-content/50 mb-1">Duration</div>
                                <div class="font-bold text-neutral text-xl leading-none">{{ $package->duration }}</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-4 bg-base-200/50 p-6 rounded-2xl hover:bg-base-200 transition-colors">
                            <div class="w-14 h-14 rounded-xl bg-white shadow-sm flex items-center justify-center text-accent shrink-0">
                                <i class="bi bi-pin-map-fill text-2xl"></i>
                            </div>
                            <div>
                                <div class="text-xs font-bold uppercase tracking-widest text-base-content/50 mb-1">Destination</div>
                                <div class="font-bold text-neutral text-xl leading-none">{{ $package->destination->name ?? 'N/A' }}</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-4 bg-base-200/50 p-6 rounded-2xl hover:bg-base-200 transition-colors col-span-1 sm:col-span-2">
                            <div class="w-14 h-14 rounded-xl bg-white shadow-sm flex items-center justify-center text-secondary shrink-0">
                            ...
                            <div>
                                ...
                                <div class="font-bold text-neutral text-xl leading-none">{{ $package->updated_at->format('F d, Y') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
...
@push('scripts')
<script>
function shareItineraryWhatsapp(type = 'customer') {
    let text = "";
    const isCustomer = type === 'customer';
    
    text += "\u2708\uFE0F *" + (isCustomer ? "YOUR GROUP TOUR PROPOSAL" : "COMPANY ITINERARY (INTERNAL)") + "* \u2708\uFE0F\n\n";
    text += "\uD83D\uDCDD *Package:* {{ str_replace('\"', '\\\"', $package->name) }}\n";
    text += "\u23F3 *Duration:* {{ $package->duration }}\n";
    text += "\uD83D\uDCCD *Destination:* {{ $package->destination->name ?? 'N/A' }}\n\n";
    ...
    @foreach($enrichedItinerary as $day)
        ...
        @if(!empty($day['hotel']) && !empty($day['hotel']['name']))
            text += "   \uD83C\uDFE8 *Stay:* {{ str_replace('\"', '\\\"', $day['hotel']['name']) }}";
            @if(!empty($day['hotel']['type'])) text += " ({{ $day['hotel']['type'] }})"; @endif
            if (!isCustomer) {
                @if(isset($day['hotel']['price_per_night'])) text += " - {{ $costBreakdown['currency'] }} {{ number_format($day['hotel']['price_per_night'], 2) }}"; @endif
            }
            text += "\n";
        @endif
        ...
        @if(!empty($day['places']))
            ...
            text += "   \uD83D\uDCCD *Visit:* {{ str_replace('\"', '\\\"', implode(', ', $pl)) }}\n";
        @endif
        
        @if(!empty($day['activities']))
            @foreach($day['activities'] as $activity)
                text += "   \u2728 *Activity:* {{ str_replace('\"', '\\\"', $activity['name']) }}";
                if (!isCustomer) {
                    @if(isset($activity['entry_ticket']['price'])) text += " - {{ $costBreakdown['currency'] }} {{ number_format($activity['entry_ticket']['price'], 2) }}"; @endif
                }
                text += "\n";
            @endforeach
        @endif
        ...
        @if(!empty($day['meals']))
            text += "   \uD83C\uDF7D *Meals:* B:{{ $day['meals']['breakfast'] ?? 'No' }}, L:{{ $day['meals']['lunch'] ?? 'No' }}, D:{{ $day['meals']['dinner'] ?? 'No' }}\n";
        @endif
    ...
    text += "--------------------------------\n";
    if (isCustomer) {
        text += "\uD83D\uDCB0 *Total Package Price:* *{{ $costBreakdown['currency'] }} {{ number_format($package->price > 0 ? $package->price : $costBreakdown['total'], 2) }}*\n\n";
        text += "\uD83D\uDCA1 Let us know if you would like the full PDF or have any questions!\n";
    } else {
        text += "\uD83D\uDCCA *Cost Breakdown (Company Only)*\n";
        text += "   - Hotels: {{ $costBreakdown['currency'] }} {{ number_format($costBreakdown['hotels'], 2) }}\n";
        text += "   - Transport: {{ $costBreakdown['currency'] }} {{ number_format($costBreakdown['transport'], 2) }}\n";
        text += "   - Activities: {{ $costBreakdown['currency'] }} {{ number_format($costBreakdown['activities'] + $costBreakdown['entry_tickets'], 2) }}\n";
        text += "   - *Total Net Cost:* {{ $costBreakdown['currency'] }} {{ number_format($costBreakdown['total'], 2) }}\n";
        text += "   - *Selling Price:* {{ $costBreakdown['currency'] }} {{ number_format($package->price, 2) }}\n";
    }
    
    text += "\n\uD83D\uDE4F *Thank you from the Tourliz Team!*";
