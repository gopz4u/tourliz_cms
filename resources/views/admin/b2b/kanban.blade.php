...
@section('title', 'B2B Pipeline — Kanban')
...
<style>
    ...
    .col-waiting        .kanban-col-header { background: #f59e0b; color: #fff; }
    .col-interested     .kanban-col-header { background: #3b82f6; color: #fff; }
    .col-converted      .kanban-col-header { background: #10b981; color: #fff; }
    .col-not_interested .kanban-col-header { background: #ef4444; color: #fff; }
    .col-dead           .kanban-col-header { background: #6b7280; color: #fff; }

    .col-leads          { border-top: 3px solid #6366f1; }
    .col-waiting        { border-top: 3px solid #f59e0b; }
    .col-interested     { border-top: 3px solid #3b82f6; }
    .col-converted      { border-top: 3px solid #10b981; }
    .col-not_interested { border-top: 3px solid #ef4444; }
    .col-dead           { border-top: 3px solid #6b7280; }
...
<div class="page-header mb-3">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            <h2><i class="bi bi-kanban me-2"></i>B2B Pipeline — Kanban</h2>
            <p class="text-muted mb-0 small">Drag cards between columns to update proposal stage instantly</p>
        </div>
        <div class="d-flex gap-2 align-items-center">
            <!-- View Toggle -->
            <div class="btn-group" role="group">
                <a href="{{ route('admin.b2b-itineraries.index') }}" class="btn btn-sm btn-outline-secondary">
                    <i class="bi bi-table me-1"></i>Table
                </a>
                <a href="{{ route('admin.b2b-itineraries.kanban') }}" class="btn btn-sm btn-primary">
                    <i class="bi bi-kanban me-1"></i>Kanban
                </a>
            </div>
            <a href="{{ route('admin.b2b-itineraries.create') }}" class="btn btn-sm btn-success">
                <i class="bi bi-building-add me-1"></i>New Proposal
            </a>
        </div>
    </div>
</div>
...
<div class="d-flex gap-3 mb-3 flex-wrap">
    @php
    $stageConfig = [
        'leads'          => ['label' => 'New Lead',       'icon' => 'bi-person-plus',      'color' => '#6366f1'],
        'waiting'        => ['label' => 'Waiting',        'icon' => 'bi-hourglass-split',  'color' => '#f59e0b'],
        'interested'     => ['label' => 'Interested',     'icon' => 'bi-heart',            'color' => '#3b82f6'],
        'converted'      => ['label' => 'Converted',      'icon' => 'bi-check-circle',    'color' => '#10b981'],
        'not_interested' => ['label' => 'Not Interested', 'icon' => 'bi-hand-thumbs-down', 'color' => '#ef4444'],
        'dead'           => ['label' => 'Dead',           'icon' => 'bi-x-circle',         'color' => '#6b7280'],
    ];
    @endphp
    ...
    <div class="kanban-board">
        ...
        <div class="kanban-col col-{{ $stageKey }}">
            ...
            <div class="kanban-cards"
                ...
                @forelse($columns[$stageKey] as $itinerary)
                <div class="kanban-card"
                     data-id="{{ $itinerary->id }}"
                     data-stage="{{ $stageKey }}"
                     id="card-{{ $itinerary->id }}">
                    ...
                    <div class="card-actions">
                        ...
                        <form action="{{ route('admin.b2b-itineraries.destroy', $itinerary->id) }}"
                              ...
                              onsubmit="return confirm('Delete {{ addslashes($itinerary->title) }}?')">
                            @csrf @method('DELETE')
                    ...
                    <div class="card-id">{{ $itinerary->quote_id }}</div>
                    <div class="card-title" title="{{ $itinerary->title }}">{{ $itinerary->title }}</div>
                    <div class="card-agency">
                        <i class="bi bi-building" style="font-size:.65rem;"></i>
                        {{ $itinerary->agency->company_name ?? 'N/A' }}
                    </div>
                    <div class="card-dest">
                        <i class="bi bi-geo-alt" style="font-size:.65rem;"></i>
                        {{ $itinerary->destination->name ?? '—' }}
                        &bull; {{ $itinerary->duration_days }}D
                    </div>

                    <div class="card-meta">
                        <span class="card-price">{{ $itinerary->currency }} {{ number_format($itinerary->total_price, 0) }}</span>
                        <span class="card-pax"><i class="bi bi-people"></i> {{ $itinerary->adults }}A
                            @if(($itinerary->children_2_6 + $itinerary->children_6_11) > 0)
                                +{{ $itinerary->children_2_6 + $itinerary->children_6_11 }}C
                            @endif
                        </span>
                        @if($itinerary->user)
                        <span class="card-assigned" title="Managed by {{ $itinerary->user->name }}">
                            <i class="bi bi-person-badge" style="font-size:.6rem;"></i> {{ $itinerary->user->name }}
                        </span>
                        @endif
                    </div>
                    ...
                    <div class="card-followup">
                        <i class="bi bi-alarm"></i> Followup: {{ $itinerary->next_followup_date->format('d M') }}
                    </div>
                    @endif

                    <div class="card-date">Updated {{ $itinerary->updated_at->diffForHumans() }}</div>
                </div>
                @empty
...
(function () {
    ...
    const UPDATE_URL = (id) => `/admin/b2b-itineraries/${id}/status`;
    ...
    function ensureEmptyEl(stageKey) {
        ...
        if (!emptyEl) {
            ...
            emptyEl.innerHTML = '<i class="bi bi-inbox" style="font-size:1.5rem;display:block;margin-bottom:6px;"></i>No proposals here yet';
            ...
            onEnd: function (evt) {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        showToast('✓ Moved to ' + (labels[newStage] || newStage));
                    } else {
