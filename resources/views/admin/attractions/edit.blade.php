...
@section('content')
    <div class="page-header">
        <h1 class="mb-0"><i class="bi bi-pencil"></i> Edit Attraction</h1>
        <p class="text-muted mb-0">Update attraction information</p>
    </div>
            ...
            <form id="attraction-form">
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save"></i> Update Attraction
                    </button>
                </div>
            </form>
        </div>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                function loadAttraction() {
                    $.get(`/admin/attractions/${attractionId}`, function (attraction) {
                        // Populate form fields
                        $('#name').val(attraction.name);
                        $('#slug').val(attraction.slug);
                        $('#short_description').val(attraction.short_description);

                        // Set Quill editor content
                        if (descriptionEditor && attraction.description) {
                            descriptionEditor.root.innerHTML = attraction.description;
                        }

                        $('#destination_id').val(attraction.destination_id || '');
                        $('#package_id').val(attraction.package_id || '');
                        $('#price').val(attraction.price || '');
                        $('#offer_price').val(attraction.offer_price || '');
                        $('#price_2_6').val(attraction.price_2_6 || '');
                        $('#price_6_10').val(attraction.price_6_10 || '');
                        $('#currency').val(attraction.currency || 'INR');
                        $('#announcement_date').val(attraction.announcement_date || '');
                        $('#total_pax').val(attraction.total_pax || '');
                        $('#is_active').prop('checked', attraction.is_active !== undefined ? attraction.is_active : (attraction.status !== undefined ? attraction.status : true));
                        ...
                        // Load featured image
                        if (attraction.image) {
                            $('#image_path').val(attraction.image);
                            $('#image-preview').attr('src', `/storage/${attraction.image}`);
                            $('#image-preview-container').show();
                        }

                        // Load gallery images
                        if (attraction.gallery && Array.isArray(attraction.gallery) && attraction.gallery.length > 0) {
                            attraction.gallery.forEach(function (path) {
                                galleryImages.push(path);
                                addGalleryImage(`/storage/${path}`, path);
                            });
                            updateGalleryPaths();
                            $('#gallery-preview-container').show();
                        }
                    }).fail(function () {
                        alert('Error loading attraction data');
                ...
                $('#attraction-form').on('submit', function (e) {
                    ...
                    $.ajax({
                        ...
                        error: function (xhr) {
                            let errorMsg = 'Error updating attraction';
                            if (xhr.responseJSON && xhr.responseJSON.message) {
                                errorMsg = xhr.responseJSON.message;
                            }
                            alert(errorMsg);
