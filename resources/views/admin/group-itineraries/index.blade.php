...
@section('content')
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h2><i class="bi bi-people-fill me-2"></i>Group Proposals</h2>
                <p class="text-muted mb-0">Manage dynamic group packages and customized group tours</p>
            </div>
            <a href="{{ route('admin.group-itineraries.create') }}" class="btn btn-primary">
                <i class="bi bi-plus-circle me-2"></i>New Group Itinerary
            </a>
        </div>
    </div>
    ...
    <div class="card mb-3">
        <div class="card-body bg-light border-bottom">
            <form action="{{ route('admin.group-itineraries.index') }}" method="GET" class="row g-2 align-items-end">
                <div class="col-md-3">
                    <label class="form-label small fw-bold">Search (Name, Phone, ID)</label>
                    <input type="text" name="search" class="form-control form-control-sm" placeholder="e.g. GRP-2024..."
                ...
                <div class="col-md-3 d-flex gap-2">
                    ...
                    <a href="{{ route('admin.group-itineraries.index') }}"
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-end pe-3">
                                    <div class="btn-group shadow-sm border rounded">
                                        <a href="{{ route('admin.group-itineraries.edit', $it->id) }}"
                                            class="btn btn-sm btn-white" title="Edit Builder">
                                            <i class="bi bi-pencil-square text-primary"></i>
                                        </a>
                                        <a href="{{ route('admin.group-itineraries.pdf', $it->id) }}" class="btn btn-sm btn-white"
                                            title="PDF">
                                            <i class="bi bi-file-pdf text-danger"></i>
                                        </a>
                                        <form action="{{ route('admin.group-itineraries.destroy', $it->id) }}" method="POST"
                                            class="d-inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-white"
                                                onclick="return confirm('Delete Proposal?')"><i
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-5 text-muted">No group itineraries created yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
