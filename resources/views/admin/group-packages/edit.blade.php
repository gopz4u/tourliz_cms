...
@section('content')
    <div class="page-header">
        <h1 class="mb-0"><i class="bi bi-pencil"></i> Edit Group Package</h1>
        <p class="text-muted mb-0">Update group package information</p>
    </div>

    <div class="card">
        <div class="card-body">
            <form id="group-package-form">
                <div class="row">
                    <div class="col-md-8">
                        <div class="row">
                            <div class="col-md-4">
                        ...
                        <small class="form-text text-muted mb-3 d-block">Choose a destination to associate this package
                            with</small>
                ...
                <div class="d-flex justify-content-between">
                    <div>
                        <a href="{{ route('admin.group-packages.index') }}" class="btn btn-secondary me-2">
                            <i class="bi bi-arrow-left"></i> Cancel
                        </a>
                        <a href="/admin/group-package-itineraries/{{ $id }}/edit" class="btn btn-info text-white">
                            <i class="bi bi-calendar3"></i> Manage Itinerary
                        </a>
                    </div>
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save"></i> Update Group Package
                    </button>
                </div>
            </form>
        </div>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                $.get('{{ route("admin.group-packages.show", $id) }}', function (pkg) {
                    ...
                    loadDestinations(pkg.destination_id);
                ...
                function loadDestinations(selectedId = null) {
                    ...
                    $.ajax({
                        ...
                        success: function (countries) {
                            ...
                            if (selectedId) {
                                ...
                                $.ajax({
                                    url: '{{ route("admin.destinations.show", "") }}/' + selectedId,
                                    type: 'GET',
                                    dataType: 'json',
                            ...
                            data.assets.forEach(hotel => {
                                if (hotel.rooms && hotel.rooms.length > 0) {
                                    hotel.rooms.forEach(room => {
                                        foundAnyRoom = true;
                                        const selected = (typeof preSelectedAssetId !== 'undefined' && preSelectedAssetId == room.id) ? 'selected' : '';
                ...
                $('#group-package-form').on('submit', function (e) {
                    e.preventDefault();
                    ...
                    $.ajax({
                        ...
                        data: formData,
                        success: function (response) {
                            alert('Group package updated successfully!');
                            window.location.href = '{{ route("admin.group-packages.index") }}';
                        },
                        error: function (xhr) {
                            let errorMsg = 'Error updating group package';
                            if (xhr.responseJSON) {
