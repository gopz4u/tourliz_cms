...
@section('content')
    <div class="page-header d-flex justify-content-between align-items-center">
        <div>
            <h1 class="mb-0"><i class="bi bi-people"></i> Group Packages</h1>
            <p class="text-muted mb-0">Manage group tour packages</p>
        </div>
        <a href="{{ route('admin.group-packages.create') }}" class="btn btn-primary">
            <i class="bi bi-plus-circle"></i> Add New Group Package
        </a>
    </div>
    ...
    @push('scripts')
        <script>
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (packages.length === 0) {
                            tbody.append('<tr><td colspan="10" class="text-center">No group packages found</td></tr>');
                            return;
                        }
                        ...
                        packages.forEach(function (pkg) {
                            ...
                            const placeName = (pkg.destination && pkg.destination.name) ? pkg.destination.name : '<span class="text-muted">-</span>';
                            ...
                            const row = `
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="/admin/group-package-itineraries/${pkg.id}/edit" class="btn btn-sm btn-info btn-action text-white" title="Manage Itinerary">
                                                            <i class="bi bi-calendar3"></i>
                                                        </a>
                                                        <a href="/admin/group-packages/${pkg.id}/edit" class="btn btn-sm btn-primary btn-action">
                    ...
                    error: function (xhr, status, error) {
                        console.error('Error loading group packages:', xhr, status, error);
                        let errorMsg = 'Error loading group packages';
            ...
            function deletePackage(id) {
                if (!confirm('Are you sure you want to delete this group package?')) return;
