...
class GroupPackageItineraryController extends Controller
{
    /**
    ...
    {
        $packages = GroupPackage::with('destination')
            ->whereNotNull('itinerary')
            ->orderBy('updated_at', 'desc')
            ->paginate(20);

        return view('admin.group-package-itineraries.index', compact('packages'));
    }
    ...
    {
        $package = GroupPackage::with(['destination', 'destination.countryRel'])->findOrFail($id);
    ...
    {
        $package = GroupPackage::findOrFail($id);
        ...
        return redirect()->route('admin.group-package-itineraries.edit', $id)
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('admin.group-package-itineraries.index')
    ...
    {
        $package = GroupPackage::with('destination')->findOrFail($id);
        ...
        return view('admin.group-package-itineraries.preview', compact('package', 'enrichedItinerary', 'costBreakdown'));
    }
    ...
    {
        $package = GroupPackage::with('destination')->findOrFail($id);
        $isCustomer = $request->query('for') === 'customer' || $request->query('type') === 'customer';
        ...
        // For customer view, we might want to use the official package price instead of calculated cost
        $totalPrice = $package->price > 0 ? $package->price : $costBreakdown['total'];

        $pdf = Pdf::loadView('admin.group-package-itineraries.pdf', compact('package', 'enrichedItinerary', 'costBreakdown', 'isCustomer', 'totalPrice'));

        $filename = ($isCustomer ? 'Proposal_' : 'Itinerary_') . str_replace(' ', '-', strtolower($package->name)) . '_' . now()->format('d_M_Y') . '.pdf';
